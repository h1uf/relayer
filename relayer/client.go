...
func MsgUpdateClient(
	...
	dcs := dstClientState.(*tmclient.ClientState)
	...
	eg.Go(func() error {
		return retry.Do(func() error {
			...
			src.log.Debug(
				...
				zap.String("chain_id", src.ChainID()),
				zap.String("client_id", src.ClientID()),
				zap.Int64("height", int64(dcs.LatestHeight.GetRevisionHeight())+1),
			)
			dstTrustedHeader, err = src.ChainProvider.QueryIBCHeader(egCtx, int64(dcs.LatestHeight.GetRevisionHeight())+1)
			return err
		}, retry.Context(egCtx), RtyAtt, RtyDel, RtyErr, retry.OnRetry(func(n uint, err error) {
			src.log.Error(
				"Failed to query IBC header when building update client message",
	...
	if err := retry.Do(func() error {
		var err error
		updateHeader, err = src.ChainProvider.MsgUpdateClientHeader(srcHeader, dcs.LatestHeight, dstTrustedHeader)
		return err
	}, retry.Context(ctx), RtyAtt, RtyDel, RtyErr, retry.OnRetry(func(n uint, err error) {
		src.log.Info(
